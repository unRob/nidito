...
@milpa.shell.export CONSUL_HTTP_TOKEN "$(nidito config get service:consul token)"
@milpa.shell.export VAULT_TOKEN "$(nidito config get dc:${NIDITO_DC:-casa} vault.root_token)"
