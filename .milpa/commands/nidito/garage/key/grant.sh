...
key_id="$(milpa nidito garage key info "$MILPA_ARG_KEY" | jq -r .id)" || @milpa.fail "Could not resolve bucket id for bucket named $MILPA_ARG_BUCKET"
...
@garage.curl "bucket/${MILPA_ARG_OPERATION}" -X POST -d @<(
  ...
  --argjson "write" "${MILPA_OPT_READ_ONLY:+false}" \
  '{
    bucketId: $bucket,
    accessKeyId: $key,
    permissions: {
