...
while read -r host service; do
  ...
  @config.write_secret "host:$host" "tls.$service" "$value" || @milpa.fail "could not save cert for $key"
...
@config.write_secret service:ca key "$(jq -r '.ca.key' output.json)" || @milpa.fail "could not save CA key"
@config.write_secret service:ca cert "$(jq -r '.ca.cert' output.json)" || @milpa.fail "could not save CA cert"
